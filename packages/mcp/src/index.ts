...
export const lokiResources = [
  {
    ...
    text: [
      ...
      "createRealtimeRoom: integrate the game's existing simulation through its predict/interpolate/extrapolate/blendCorrection/shouldCorrect/composeRenderState callbacks instead of a parallel input queue, RTT estimator, snapshot pacer, stale-round rejection, input ledger, interpolation buffer, or reconnect netcode; the SDK owns per-submission in-flight accounting and setInput() network pacing too. For multi-entity games, use getRenderStates()/composeRenderState (or the createEntityCompositor/createLocalPrediction helpers, preferring the bulk setEntities over setEntity for many entities) to render each entity from the stream that fits it (predicted for local, interpolated/latestAuthoritative for remotes) instead of predicting every entity, which diverges after collisions. publishSnapshot() already coalesces multiple same-turn calls to a single send; do not build a separate coalescing layer. Use sendEffect()/onConfirmedEffect() to confirm authoritative events (e.g. collisions) with a stable id instead of a custom event-confirmation channel, so speculative local particles/audio can be deduped against the confirmed outcome. Use createHostedLokiClient() to own the hosted-shell loki:init/loki:session handshake at page boot instead of hand-writing it. Keep one game-owned render loop, keep authoritative snapshots compact and self-contained. snapshotHz (default/cap 30) is a ceiling, not a delivery guarantee: start conservative with adaptiveRate: true and a low initialSnapshotHz, or use calibrateRealtimeRoom() against a real two-player pair to pick a profile, rather than defaulting to the highest rate; game.json tickRate is separate and does not need to match snapshotHz. Report the selected rates plus observed diagnostics (including acceptance/rejection ratios and onDiagnosticWarning events) as evidence. Loki does not supply physics, collision, rendering optimization, or competitive integrity for realtime rooms.",
...
export async function callLokiTool(
  ...
  if (name === "integration_requirements") {
    return {
      packages: ["@lokiplay/sdk@0.3.7", "@lokiplay/ui-web@0.3.7"],
      command: "npm install @lokiplay/sdk@0.3.7 @lokiplay/ui-web@0.3.7",
  ...
  if (name === "diagnose_multiplayer") {
    ...
    if (!/@lokiplay\/sdk|FirstPartyTransport|LokiClient/.test(joined)) {
      findings.push({
        ...
        message: "Install and initialize @lokiplay/sdk@0.3.7.",
      });
    }
    if (
