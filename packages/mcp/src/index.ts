...
export async function callLokiTool(
  ...
  if (name === "integration_requirements") {
    return {
      packages: ["@lokiplay/sdk@0.3.2", "@lokiplay/ui-web@0.3.2"],
      command: "npm install @lokiplay/sdk@0.3.2 @lokiplay/ui-web@0.3.2",
  ...
  if (name === "diagnose_multiplayer") {
    ...
    if (!/@lokiplay\/sdk|FirstPartyTransport|LokiClient/.test(joined)) {
      findings.push({
        ...
        message: "Install and initialize @lokiplay/sdk@0.3.2.",
      });
    }
    if (
